...
internal class BeansRepositoryTests
{
    ...
    [Test]
    [Description("Bean entity should be successfully updated with provided properties")]
    public async Task BeanEntity_ShouldBe_SuccessfullyUpdatedWithProvidedProperties()
    {
        var bean = new Bean()
        {
            Name = "TURNABOUT",
        ...
        var beanDto = new BeanDTO()
        {
            ...
            IsBOTD = false,
            Cost = 100M,
            ImageName = "photo-updated",
            Colour = BeanColour.Green,
            Index = 5,
        };

        await BeansRepository.UpdateAsync(bean.Id, beanDto, newCountry.Id);
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(storedBean.Id, Is.EqualTo(bean.Id));
            ...
            Assert.That(storedBean.CountryId, Is.EqualTo(newCountry.Id));
        }
    }
