...
internal class ExternalDependenciesTests
{
    ...
    [Test]
    [Description("Get should return correct response")]
    public async Task Get_Should_ReturnCorrectResponse()
    {
        using var scope = _factory.Services.CreateScope();
        ...
        using var response = await httpClient.GetAsync($"/beans/{seededBean.Id}");
